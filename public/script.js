...
class GlaamWebsite {
    constructor() {
        ...
        this.productTranslationMap = {
            ...
            43: { name: 'products.seasonal.classicSpring', desc: 'products.seasonal.classicSpringDesc' },
            44: { name: 'products.seasonal.brightSummer', desc: 'products.seasonal.brightSummerDesc' },
            45: { name: 'products.seasonal.warmAutumn', desc: 'products.seasonal.warmAutumnDesc' },
            46: { name: 'products.seasonal.coolWinter', desc: 'products.seasonal.coolWinterDesc' },
    ...
    initWeddingPackages() {
        return [
            ...
            },
            {
                ...
                description: 'Boutonnière za ženina in svatove.',
                price: 45,
                category: 'wedding',
                icon: 'fas fa-user-tie',
                includes: ['Boutonnière za ženina', 'Boutonnière za svatove']
            },
            {
                id: 106,
                ...
                category: 'wedding',
                icon: 'fas fa-cross',
                includes: ['Cerkvena dekoracija', 'Oltar dekoracija', 'Pew dekoracija']
            }
        ];
    }
    ...
    renderProducts(filter = 'romantic') {
        ...
        productsGrid.innerHTML = filteredProducts.map(product => {
            const isCustomMode = filter === 'custom' && product.category === 'custom';
            
            return `
            <div class="product-card fade-in ${isCustomMode ? 'custom-flower' : ''}" data-category="${product.category}" onclick="glaam.selectProduct(${product.id})">
                ...
                <div class="product-info">
                    ...
                    ${!isCustomMode ? `<p class="product-description">${this.getTranslatedDesc(product)}</p>` : ''}
                    ${isCustomMode ? 
                        // Custom bouquet mode - single flower selection
                        `<div class="custom-flower-info">
                            <div class="single-flower-price">${product.price}€ ${typeof i18next !== 'undefined' ? i18next.t('products.pricePerFlower') || 'za rožo' : 'za rožo'}</div>
                            <button class="add-flower-btn" onclick="event.stopPropagation(); glaam.addToCart(${product.id})">
                                <i class="fas fa-plus"></i> ${typeof i18next !== 'undefined' ? i18next.t('products.addFlowerToBouquet') || 'Dodaj rožo v šopek' : 'Dodaj rožo v šopek'}
                            </button>
                        </div>` :
                        // Regular mode - quantity controls
                    ...
                    <div class="product-footer">
                        <span class="product-price">${product.price}€</span>
                            <button class="product-btn" onclick="event.stopPropagation(); glaam.addToCart(${product.id})">
...
*/
    ...
    applyFilter(filter) {
        ...
        // Show/hide custom size buttons
        const customSizeButtons = document.getElementById('customSizeButtons');
        console.log('Custom size buttons element:', customSizeButtons);
        console.log('Current filter:', filter);
        if (customSizeButtons) {
            if (filter === 'custom') {
                console.log('Showing custom size buttons');
                customSizeButtons.style.display = 'flex';
            } else {
                console.log('Hiding custom size buttons');
                customSizeButtons.style.display = 'none';
            }
        } else {
            console.error('Custom size buttons element not found!');
        }
    ...
    addToCart(productId) {
        ...
        if (product) {
            // Check if we're in custom bouquet mode and this is a custom category product
            const isCustomMode = document.querySelector('.filter-btn[data-filter="custom"]')?.classList.contains('active');
            
            if (isCustomMode && product.category === 'custom') {
                // Add to custom bouquet instead of regular cart
                const success = this.addFlowerToCustomBouquet(productId);
                if (!success) {
                    return; // Error message already shown in addFlowerToCustomBouquet
                }
            } else {
    ...
    addFlowerToCustomBouquet(productId) {
        ...
        // Check if bouquet is full
        if (customBouquet.selectedFlowers.length >= customBouquet.maxItems) {
            const maxDisplay = customBouquet.maxItems === 999 ? '∞' : customBouquet.maxItems;
            this.showNotification(`Šopek je poln! Maksimalno ${maxDisplay} rož.`, 'warning');
            return false;
        }
        ...
        const maxDisplay = customBouquet.maxItems === 999 ? '∞' : customBouquet.maxItems;
        this.showNotification(`${product.name} dodana v šopek! (${customBouquet.selectedFlowers.length}/${maxDisplay})`, 'success');
